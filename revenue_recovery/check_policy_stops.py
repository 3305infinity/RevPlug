...
print(f"Actionable Opportunity (wrong) = {eligible_wrong} * {totalAtRisk}/{totalCases} = {opp_wrong} = ₹{opp_wrong/100:.2f}")
print(f"\nFixed: eligibleCases = {totalCases} - {no_action} = {eligible_fixed}")
print(f"Actionable Opportunity (fixed) = {eligible_fixed} * {totalAtRisk}/{totalCases} = {opp_fixed} = ₹{opp_fixed/100:.2f}")

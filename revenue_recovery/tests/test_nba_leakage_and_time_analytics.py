...
def test_revenue_leakage_analytics():
    ...
    auth_cat = next(c for c in report.categories if c["category_id"] == "auth")
    assert auth_cat["category_label"] == "Authentication Failures (3DS)"
    assert auth_cat["recovery_rate_pct"] > 50.0
    assert "UPI Payment Link" in auth_cat["recommended_policy_change"]

...
def test_stress_large_batch():
    ...
    # Verify bounds
    assert result.revplug.total_interventions <= 1000, f"Expected bounded interventions, got {result.revplug.total_interventions}"
    ...
    # Verify ledgers
    assert result.revplug.actual_recovered <= result.revplug.total_amount_at_risk, "Double-counting detected: recovered amount exceeds amount at risk"
    
    # Ensure all cases were processed without unhandled exceptions
    assert result.revplug.recovered_count >= 0
    ...
    print(f"Total at risk: {result.revplug.total_amount_at_risk}")
    print(f"Total recovered: {result.revplug.actual_recovered}")

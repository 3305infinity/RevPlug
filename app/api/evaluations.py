...
def api_benchmark_summary() -> Response:
    ...
    from pathlib import Path
    try:
        report_path = Path(__file__).resolve().parent.parent.parent / "artifacts" / "evaluation_report.json"
    ...
    return JSONResponse(status_code=200, content={
        ...
        "multi_seed": {
            ...
            "revplug_mean_cost": agg.get("revplug_mean_cost") if agg.get("revplug_mean_cost") is not None else ((agg.get("revplug_mean_gross", 0) - agg.get("revplug_mean_net", 0)) if agg.get("revplug_mean_gross") is not None and agg.get("revplug_mean_net") is not None else 0),
            "revplug_mean_cost_rs": fmt_minor(agg.get("revplug_mean_cost") if agg.get("revplug_mean_cost") is not None else ((agg.get("revplug_mean_gross", 0) - agg.get("revplug_mean_net", 0)) if agg.get("revplug_mean_gross") is not None and agg.get("revplug_mean_net") is not None else 0)),
            "safe_mean_cost": agg.get("safe_mean_cost") if agg.get("safe_mean_cost") is not None else ((agg.get("safe_mean_gross", 0) - agg.get("safe_mean_net", 0)) if agg.get("safe_mean_gross") is not None and agg.get("safe_mean_net") is not None else 0),
            "safe_mean_cost_rs": fmt_minor(agg.get("safe_mean_cost") if agg.get("safe_mean_cost") is not None else ((agg.get("safe_mean_gross", 0) - agg.get("safe_mean_net", 0)) if agg.get("safe_mean_gross") is not None and agg.get("safe_mean_net") is not None else 0)),
            "naive_mean_cost": agg.get("naive_mean_cost") if agg.get("naive_mean_cost") is not None else ((agg.get("naive_mean_gross", 0) - agg.get("naive_mean_net", 0)) if agg.get("naive_mean_gross") is not None and agg.get("naive_mean_net") is not None else 0),
            "naive_mean_cost_rs": fmt_minor(agg.get("naive_mean_cost") if agg.get("naive_mean_cost") is not None else ((agg.get("naive_mean_gross", 0) - agg.get("naive_mean_net", 0)) if agg.get("naive_mean_gross") is not None and agg.get("naive_mean_net") is not None else 0)),
